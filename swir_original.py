...
# 기본 설정
args = {
    "data_folder": "D:/waper/original_data/",
    "graphs_folder": "D:/waper/original/graph/",
    "model_save_folder": "D:/waper/original/trained_model/",
    "epoch": 50,
    "batch_size": 1,
    "learning_rate": 1e-4,
    "img_size": (1032, 1296),
...
##### 모델 로드
base_model = ResNet50(weights='imagenet', include_top=True, input_tensor=Input(shape=(1032, 1296, 3)))      # 원본 이미지
...
model_checkpoint = ModelCheckpoint(filepath=args["model_save_folder"] + 'original.h5', save_best_only=True, monitor='val_loss', verbose=1)
...
plt.savefig(args["graphs_folder"] + 'original' + f"acc.png")
plt.show()
...
plt.ylabel('Loss')
plt.legend(['train', 'valid'], loc='upper left')
plt.savefig(args["graphs_folder"] + 'original' + f"loss.png")
plt.show()
...
def evaluate_model(model, test_generator):
    ...
    plt.savefig(args["graphs_folder"] + 'original' + f"auc.png")
    plt.show()
